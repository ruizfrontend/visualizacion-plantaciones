...
return function(){e||(e=!0,setTimeout(function(){t.call(),e=!1},a))}}function numberThousandsDots(t){return(""+t).replace(/(\d)(?=(\d\d\d)+(?!\d))/g,"$1.")}function firstCapital(t){return t.charAt(0).toUpperCase()+t.slice(1)}function millionsParser(t){var a=t/1e6
...
for(var f=h.append("g").attr("class","lines"),g=h.append("g").attr("class","dots"),x=0;c>=x;x++){var k=r+l*x,$=p+i(a.expByYear[pG.initYear+x]),C=p+i(a.impByYear[pG.initYear+x])
d+=k+" "+(n+p)+" L",u+=k+" "+(n+p)+" L",G+=k+" "+C+" L",v+=k+" "+C+" L",m+=k+" "+$+" L",y+=k+" "+$+" L",g.append("circle").attr("class","circLinesWhite").attr("cx",k).attr("cy",$).attr("r",4).attr("opacity",0).transition().duration(2e3).attr("opacity",1),g.append("circle").on("mouseenter",pG.evs.circGraphTooltip).on("mouseout",pG.evs.circGraphTooltipOut).attr("class","circLines linesExp activaYear"+(pG.initYear+x==pG.act.year?" linesExpFill":"")).attr("data-year",pG.initYear+x).attr("cx",k).attr("cy",$).attr("r",3).attr("stroke","#126567").attr("title",a.actCountry.name+" importó "+pG.tools.parseAmmount(a.expByYear[pG.initYear+x])+" de "+pG.txt.productos[pG.act.prod]+" en "+(pG.initYear+x)+".").attr("opacity",0).transition().duration(2e3).attr("opacity",1),g.append("circle").attr("class","circLinesWhite").attr("cx",k).attr("cy",C).attr("r",4).attr("opacity",0).transition().duration(2e3).attr("opacity",1),g.append("circle").on("mouseenter",pG.evs.circGraphTooltip).on("mouseout",pG.evs.circGraphTooltipOut).attr("class","circLines linesImp activaYear"+(pG.initYear+x==pG.act.year?" linesImpFill":"")).attr("data-year",pG.initYear+x).attr("cx",k).attr("cy",C).attr("r",3).attr("title",a.actCountry.name+" exportó "+pG.tools.parseAmmount(a.impByYear[pG.initYear+x])+" de "+pG.txt.productos[pG.act.prod]+" en "+(pG.initYear+x)+".").attr("opacity",0).transition().duration(2e3).attr("opacity",1)}d+=r+o+" "+(n+p)+" L"+r+" 0 Z",G+=r+o+" "+(n+p)+" Z",m+=r+o+" "+(n+p)+" Z",f.append("path").attr("d",d).attr("stroke","none").attr("fill","rgba(211, 93, 51, 0.1)").transition().duration(1e3).attr("d",G),f.append("path").attr("d",d).attr("stroke","none").attr("fill","rgba(150, 203, 81, 0.1)").transition().duration(1e3).attr("d",m),f.append("path").attr("d",u.slice(0,-1)).attr("stroke","rgb(211, 93, 51)").attr("stroke-width","1").attr("fill","none").transition().duration(1e3).attr("d",v.slice(0,-1)),f.append("path").attr("d",u.slice(0,-1)).attr("stroke","#126567").attr("stroke-width","1").attr("fill","none").transition().duration(1e3).attr("d",y.slice(0,-1))}},evs:{circGraphTooltipOut:function(){$("#pop .tooltip").remove()},circGraphTooltip:function(t,a){var e=$(this),r=$("#pop"),p=e.offset().left-r.offset().left,o=r.height()-(e.offset().top-r.offset().top),n=$('<div class="tooltip top" role="tooltip"><div class="tooltip-arrow"></div><div class="tooltip-inner">'+e.attr("title")+"</div></div>")
